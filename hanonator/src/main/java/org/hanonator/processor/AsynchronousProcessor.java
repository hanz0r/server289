 ...
 * @param <T>
 */
public abstract class AsynchronousProcessor<T, R> implements Processor<T, R> {
	...
	@Override
	public List<R> process(Collection<T> collection) throws GameException {
		try {
			...
			if (!collection.isEmpty()) {
				final CyclicBarrier barrier = new CyclicBarrier(collection.size() + 1);
				...
				for (Iterator<T> iterator = collection.iterator(); iterator.hasNext(); ) {
					...
					service.submit(() -> {
						try {
							 ...
							 */
							output.add(this.processElement(element));
